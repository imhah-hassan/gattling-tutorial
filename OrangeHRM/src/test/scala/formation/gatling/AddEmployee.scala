...
class AddEmployee extends Simulation {
    ...
    .exec(http("login")
		...
		.exec(http("viewEmployeeList")
			...
			.check(css("#resultTable>tbody>tr:nth-of-type(2)>td:nth-of-type(1)>input[type='checkbox']", "value").saveAs("empId"))
			.check(css("#resultTable>tbody>tr:nth-of-type(2)>td:nth-of-type(2)>a").saveAs("empNumber"))
